import { getSavedCartIDs, saveCartID, removeCartID } from './cartFunctions';
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async (event) => {
    ...
    const idEl = targetEl.parentNode.firstChild.innerText;
    const cart = document.querySelector('.cart__products');
    const product = await fetchProduct(idEl);
    console.log(product);
    saveCartID(product.id);
    const productInfo = { id: product.id,
      title: product.title,
      price: product.price,
    ...
    const productEl = createCartProductElement(productInfo);
